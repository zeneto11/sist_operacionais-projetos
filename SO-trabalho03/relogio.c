...
int frame; //funciona como o ponteiro
...
void load_page(Page page) {
    ...
    //seta a variável global frame para 0 se ele atingir o número de frames, funcionando como uma lista circular
    if (frame >= NUM_FRAMES){
        frame = 0;
    }
    ...
    while (memory[frame].page.bit==1){
        ...
        //depois que o ponteiro passar por ele o bit vira 0
    ...
    //se chegar aqui ele achou o bit 0
    ...
    memory[frame].ocupado = 1;
    memory[frame].page = page;
    printf("Página %d do processo %d colocada no frame %d\n", page.pag_num, page.process.processo_id, frame);
    frame++;
    gerenciador_memoria();
}
